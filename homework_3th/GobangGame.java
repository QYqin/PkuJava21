 ...
 * 2.如果电脑在四个棋子连线的情况下不能下棋，则设置变量,查看是否有三个棋子相连的情况。
 * 3.如果三个棋子依然不能下棋，则在玩家周围下棋。
 * 4.对于玩家下棋，判断最大连线的数量白棋是否少于黑棋，如果少于黑棋，在再黑棋的两端下棋，如果最大连线方向的两端无法下棋，则在白棋两端下棋。
 * 5.如果黑棋和白棋的数量相等，或者白棋的连线数量大于黑棋，在白棋的两端下棋。
 * 6.如果黑棋和白棋的最大连线方向都无法下棋，则在白棋的最大值周围下棋，如果依然无法下棋，则随机下棋。
 * 7.通过设置返回值及连线方向标志位来对以上情况进行判断。
 * 
*/
while(true)
{
	...
	*/
		...
		result[1]=MaxCountArr[1];	
		//如果返回坐标为-1，则此位置不能下棋，则重新选择。
		System.out.println("BLACKSIGN"+BLACKSIGN);
		if(MaxCountArr[0]<0||MaxCountArr[1]<0)
		{
			BLACKSIGN=-1;
			continue;
